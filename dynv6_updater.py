    ...
    def get_ipv6_address(self):
        try:
            ...
            if result.returncode == 0:
                ...
                # 使用正则表达式匹配适配器段落
                adapters = re.findall(r'adapter .*?:\n(.*?)\n\n', output, re.DOTALL)
                for adapter in adapters:
                    # 检查是否包含给定的MAC地址
                    if self.mac_address in adapter:
                        # 查找IPv6地址
                        ipv6_match = re.search(r'IPv6 Address.*?:(.*?)(?:\n|$)', adapter)
                        if ipv6_match:
                            ipv6_address = ipv6_match.group(1).strip().split('(Preferred)')[0]
                            return ipv6_address
                return "未找到匹配的网卡地址"
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--daemon', required=False, default=30, help='后台运行间隔 单位秒。0 则只运行一次，默认30')

    args = parser.parse_args()

    # 创建Dynv6Updater对象并运行
    updater = Dynv6Updater(args.mac_address, args.token, args.domain)
    updater.run(interval=int(args.daemon))  # 运行，每30秒更新一次
